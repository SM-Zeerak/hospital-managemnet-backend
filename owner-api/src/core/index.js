...
export const registerCore = fp(async (app) => {
    ...
    app.decorate('syncGuard', async (request) => {
        const expectedToken = process.env.TENANT_SYNC_WEBHOOK_TOKEN;
        if (!expectedToken) {
            throw app.httpErrors.internalServerError('Sync token not configured');
        }

        const provided = request.headers.authorization || '';
        const trimmed = provided.replace(/^Bearer\s+/i, '');
        if (trimmed !== expectedToken) {
            throw app.httpErrors.unauthorized('Invalid sync token');
        }
    });
