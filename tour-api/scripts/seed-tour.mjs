...
if (values.help) {
    console.log(`
Usage: node scripts/seed-tour.mjs [options]
Platform: Tour Management System
...
Example:
  node scripts/seed-tour.mjs --tenantId abc-123 --dbName tour_operator_a
`);
    process.exit(0);
}
...
console.log(`\n🚙 Seeding Tour Operator Database`);
console.log(`================================`);
...
try {
    ...
    console.log('\n✅ Tour operator database seeded successfully!');
    console.log(`\n📝 To use this tour operator, set in .env:`);
    console.log(`   TENANT_ID=${values.tenantId}`);
    console.log(`   TENANT_PG_URL=postgres://user:pass@localhost:5432/${values.dbName}\n`);
} catch (error) {
    console.error('\n❌ Error seeding tour operator database:', error.message);
    process.exit(1);
}
