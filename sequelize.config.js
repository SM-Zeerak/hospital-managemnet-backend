...
for (const env of environments) {
    const envVar = `PG_URL${env === 'development' ? '' : `_${env.toUpperCase()}`}`;
    const url = process.env[envVar] || process.env.PG_URL || null;
    ...
    if (url) {
        try {
            ...
            // eslint-disable-next-line no-console
            console.error(`Failed to ensure database for ${env}:`, error.message);
        }
    } else {
        // eslint-disable-next-line no-console
        console.warn(`PG_URL not configured for ${env}`);
    }
